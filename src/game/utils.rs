...
pub fn find_circle_to_circle_intersections(p0: Vec2, r0: f32, p1: Vec2, r1: f32) -> [Option<Vec2>; 2] {
    ...
    {
        result[0] = Some(p0.lerp(p1, r0 / (r0 + r1)));
        return result;
    }
    ...
    // Find P2.
    let cx2 = p0.x + a * (p1.x - p0.x) / dist;
    let cy2 = p0.y + a * (p1.y - p0.y) / dist;

    // Get the points P3.
    result[0] = Some(Vec2::new(
        cx2 + h * (p1.y - p0.y) / dist,
        cy2 - h * (p1.x - p0.x) / dist,
    ));
