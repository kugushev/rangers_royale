...
fn spawn_enemy(mut commands: Commands, asset_server: Res<AssetServer>, mut texture_atlases: ResMut<Assets<TextureAtlas>>) {
    ...
    const SHIFT: f32 = 100.;
    do_spawn(Vec2::new(200. + SHIFT, 0.), CharacterOrigin::Orc);
    do_spawn(Vec2::new(200. - SHIFT, 0.), CharacterOrigin::Orc);
    do_spawn(Vec2::new(200., 0. + SHIFT), CharacterOrigin::Orc);
    do_spawn(Vec2::new(200., 0. - SHIFT), CharacterOrigin::Orc);
    do_spawn(Vec2::new(200. + SHIFT, 0. - SHIFT), CharacterOrigin::Orc);
    do_spawn(Vec2::new(200. + SHIFT, 0. + SHIFT), CharacterOrigin::Orc);
    do_spawn(Vec2::new(200. - SHIFT, 0. - SHIFT), CharacterOrigin::Orc);
    do_spawn(Vec2::new(200. - SHIFT, 0. + SHIFT), CharacterOrigin::Orc);
}